          ...
          <li class="food-list-hook" v-for="(good,index) in goods" :key="index">
            ...
            <ul>
              <li class="food-item bottom-border-1px" v-for="(food,index) in good.foods"   @click="showFood(food)"            >
                <div class="icon">
                  <img width="57" height="57" v-lazy="food.image">
                </div>
  ...
  methods:{
    ...
    _initScroll(){
      new BScroll('.menu-wrapper',{
        //betterScroll默认会阻止点击时间，下面操作开启点击事件
        click:true
      })
    ...
    _initTops(){
     ...
     const tops = [0]
      //设定一个top的初始值
      let top = 0
      //获取所有分类的li  原始方法 document.getElementsByClassName('food-list-hook')
      const lis = this.$refs.foodUl.children
      //将获取的分类li的伪数组转化成真数组遍历
      Array.prototype.slice.call(lis).forEach(li=>{
      //每遍历一个数组  top的值加上上一个li的可见区域高
      top += li.clientHeight
       //把每次处理后的top添加到tops中
        tops.push(top)
                                             })
